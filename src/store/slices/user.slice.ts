...
const defaultUserState = usersAdapter.addMany(
...
export const userSlice = createSlice({
  ...
  reducers: {
    addUser: (state) => {
      ...
      const userId = usersCount == 0 ? 1 :
        +(userIds[usersCount - 1].toString().substring(4)) + 1;
      const newUser = {
        userId: 'user' + userId,
        name: 'User ' + userId,
        unreadMessages: 0
...
export const { 
  addUser, 
  deleteUser, 
  incrementUserUnreadMessagesCount, 
  resetUserUnreadMessagesCount 
} = userSlice.actions;
